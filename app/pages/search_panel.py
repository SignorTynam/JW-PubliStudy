...
class SearchPanel(QWidget):
    ...
    def __init__(self, translations: I18n, search_service: SearchService) -> None:
        super().__init__()
    ...
    def _populate_combo(self, combo: QComboBox, options: tuple[tuple[object, str], ...], selected_value: object) -> None:
    ...
    def _on_result_selected(self) -> None:
        ...
        if selected_items:
            ...
            self._selected_result = next((result for result in self._results if result.chunk_id == chunk_id), None)
        self._update_detail()
    ...
    def _format_reference(self, result: SearchResult) -> str:
        if result.page_start is None:
            return self._translations.t("study.reference.chunk").format(title=result.publication_title, chunk=result.chunk_index + 1)
        if result.page_end is not None and result.page_end != result.page_start:
            return self._translations.t("study.reference.pages").format(title=result.publication_title, page_start=result.page_start, page_end=result.page_end)
        return self._translations.t("study.reference.page").format(title=result.publication_title, page=result.page_start)
